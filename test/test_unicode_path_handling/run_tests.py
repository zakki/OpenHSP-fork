...
# The legacy source case exercises the Windows ACP boundary.
LEGACY_ENCODING = "mbcs"
...
def test_legacy_source_and_include(hspcmp: Path, common: Path, workdir: Path) -> None:
    source_dir = workdir / "legacy-日本語"
    source_dir.mkdir()
    include = source_dir / "従来.as"
    source = source_dir / "legacy-main.hsp"
    output = source_dir / "legacy-output.ax"

    write_fixture(include, 'mes "LEGACY_INCLUDE_MARKER"\n', LEGACY_ENCODING)
    write_fixture(
        source,
        '#include "従来.as"\nmes "LEGACY_MAIN_MARKER"\nmes __file__\n',
        LEGACY_ENCODING,
    )

    run_compile(hspcmp, common, source, output, utf8_input=False)
    result = output.read_bytes()
    assert b"LEGACY_INCLUDE_MARKER" in result


def test_utf8_path_with_legacy_source(
    hspcmp: Path, common: Path, workdir: Path
) -> None:
    source_dir = workdir / "UTF8パス-日本語"
    source_dir.mkdir()
    source = source_dir / "sjis-source.hsp"
    output = source_dir / "sjis-output.ax"

    # The second byte of the CP932 character 表 is 0x5c. This distinguishes
    # SJIS string scanning from UTF-8 scanning, where it would be treated as
    # an independent backslash.
    write_fixture(
        source,
        'mes "SJIS_SOURCE_表"\nmes __file__\n',
        LEGACY_ENCODING,
    ...
    run_compile(hspcmp, common, source, output, utf8_input=False)
    result = output.read_bytes()
    output_encoding = "utf-8" if os.name == "nt" else LEGACY_ENCODING
    require_marker(
        result,
        "SJIS_SOURCE_表".encode(output_encoding),
        "SJIS source marker",
        output,
    )
    require_marker(
...
def test_dll_compile_and_pack(dll_path: Path, common: Path, workdir: Path) -> None:
    ...
    try:
        ...
        pack_dir.mkdir()
        packopt_source = pack_dir / "packopt-main.hsp"
        packopt_output = pack_dir / "packopt-start.ax"
        packopt_value = "packopt-😀"
        write_fixture(
            packopt_source,
            '#packopt name "{}"\nmes "PACKOPT_MAIN_MARKER"\n'.format(
                packopt_value
            ),
            "utf-8",
        )
        ...
        with working_directory(pack_dir):
            dll.call_path("hsc_ini", packopt_source)
            dll.call_path("hsc_compath", common)
            dll.call_path("hsc_objname", packopt_output)
            dll._check("hsc_comp", dll.hsc_comp(128, 4 | 32, 0, 0))
            packfile = pack_dir / "packfile"
            require_marker(
                packfile.read_bytes(),
                (";!name={}\r\n".format(packopt_value)).encode("utf-8"),
                "UTF-8 #packopt value",
                packfile,
            )
...
def main() -> int:
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="openhsp-unicode-path-") as temporary:
            ...
            if os.name == "nt":
                test_legacy_source_and_include(hspcmp, common, workdir)
                print("PASS legacy source/include")
                test_utf8_path_with_legacy_source(hspcmp, common, workdir)
                print("PASS UTF-8 path with legacy source")
            else:
                print("SKIP legacy source/include (Windows ACP only)")
