...
class Suite:
    def __init__(self, target: Target, output: Path, worker: Path):
        self.target = target
        self.output = output
        self.worker = worker
    ...
    def execute(self, case_id: str, operation: str, character_class: str,
        ...
        self._save(result)
        # Always retain exact source/AX/DPM/files, including successful cases.
        if work.exists():
...
def write_summary(output: Path, targets: list[Target], results: list[Result], rows: list[dict[str, str]]) -> None:
    ...
    for target in targets:
        metadata.append({"name": target.name, "runtime": str(target.runtime),
                         "runtime_bits": pe_bits(target.runtime), "compiler_dll": str(target.compiler_dll),
                         "compiler_bits": pe_bits(target.compiler_dll), "common": str(target.common),
...
def arguments() -> argparse.Namespace:
    ...
    parser.add_argument("--baseline-root", type=Path, default=Path(r"C:\hsp37"))
    parser.add_argument("--candidate-runtime", type=Path,
                        default=ROOT / "src/hsp3/Release64/hsp3cl_64.exe")
    parser.add_argument("--candidate-dll", type=Path,
                        default=ROOT / "src/hspcmp/Release64/hspcmp_64.dll")
...
def main() -> int:
    ...
    targets = [
        Target("baseline-3.7", args.baseline_root / "hsp3cl.exe",
               args.baseline_root / "hspcmp.dll", args.baseline_root / "common", "cp932", False),
        Target("candidate", args.candidate_runtime, args.candidate_dll,
               args.candidate_common, "utf-8", True),
    ]
    missing = [str(path) for target in targets for path in
               (target.runtime, target.compiler_dll, target.common) if not path.exists()]
    if missing:
    ...
    for target in targets:
        ...
        current = Suite(target, output, workers[pe_bits(target.compiler_dll)]).all()
        grouped.append(current)
        all_results.extend(current)
    rows = compare(grouped[0], grouped[1])
