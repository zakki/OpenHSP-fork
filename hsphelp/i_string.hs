...
"filename", "encoding"
"filename" : 書き込みファイル名
"encoding" : 文字コード名(省略時は従来の内部文字コード)
%inst
...
HSPUTF8環境およびUnix系環境では、"UTF-8"、"CP932"、"EUC-JP"などのICUで認識できる文字コード名を指定できます。
指定した文字コードで表現できない文字がある場合はエラーになります。
ANSI文字列を使用する通常のHSPWINでは、文字コード指定は使用できません。
...
必ず最初にnotesel命令で対象となるバッファを設定する必要があるので注意してください。
HSPUTF8環境およびUnix系環境では、"UTF-8"、"CP932"、"EUC-JP"などのICUで認識できる文字コード名を指定できます。
不正な入力や未対応の文字コード名を指定した場合はエラーになります。
ANSI文字列を使用する通常のHSPWINでは、文字コード指定は使用できません。
%href
notesel
notesave
