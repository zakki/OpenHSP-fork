...
void CToken::SetLabelListBuffer( CMemBuf *buf, int mode, char *match, int line, char *filename )
{
	...
	static char *p[] = { "---", "fnc", "mac", "lab", "var", "exv", "cmd", "exc", NULL };
	cg_labout_header = p;
}
	...
	*dummy = 0;
	...
	while ( 1 ) { // マクロ置き換え
		...
		if ( a1 == '%' ) {
				...
				*mactmp = 0;
				...
				case 's':
					...
					if ( !( 0 <= val && val <= macopt - 1 ) ) {
						SetError( "illegal macro parameter %s" );
						return 2;
					}
					w2 = mactmp;
					p = prm[val];
			...
			if ( type != TK_NUM ) {
				SetError( "macro parameter invalid" );
				return 1;
			}
			val--;
			if ( !( 0 <= val && val <= macopt - 1 ) ) {
				SetError( "illegal macro parameter" );
				return 2;
			}
			p = prm[val];
			endp = prme[val];
...
ppresult_t CToken::PP_Module( void )
{
	...
	if ( !IsGlobalMode() ) {
		SetError( "not in global mode" );
		return PPRESULT_ERROR;
	}
	if ( CheckModuleName( word ) ) {
		SetError( "bad module name" );
		return PPRESULT_ERROR;
	}
...
ppresult_t CToken::PP_Ahtmes( void )
{
	...
	while ( 1 ) {
		...
		if ( ( i != TK_OBJ ) && ( i != TK_NUM ) && ( i != TK_STRING ) ) {
			SetError( "illegal ahtmes parameter" );
			return PPRESULT_ERROR;
		}
		...
		i = GetToken();
		if ( i != '+' ) {
			SetError( "invalid ahtmes format" );
			return PPRESULT_ERROR;
		}
		addprm++;
	}
...
ppresult_t CToken::PP_PackOpt( void )
{
	...
	if ( packbuf != NULL ) {
		i = GetToken();
		if ( i != TK_OBJ ) {
			SetError( "illegal option name" );
			return PPRESULT_ERROR;
		}
		strncpy( optname, (char *)s3, 128 );
		i = GetToken();
		if ( ( i != TK_OBJ ) && ( i != TK_NUM ) && ( i != TK_STRING ) ) {
			SetError( "illegal option parameter" );
			return PPRESULT_ERROR;
		}
