...
EXPORT BOOL WINAPI hsc_comp( int p1, int p2, int p3, int p4 )
{
	...
	*/
	...
	if ( p1 & 4 )
		ppopt |= HSC3_OPT_UTF8OUT;
	if ( p1 & 256 )
		ppopt |= HSC3_OPT_EMSCRIPTEN;

	if ( p2 & 1 )
		ppopt |= HSC3_OPT_NOHSPDEF;
	if ( p2 & 4 )
		ppopt |= HSC3_OPT_MAKEPACK;
	if ( p2 & 8 )
		ppopt |= HSC3_OPT_READAHT;
	if ( p2 & 16 )
		ppopt |= HSC3_OPT_MAKEAHT;
	if ( p2 & 32 )
	...
	if ( p1 & 8 )
		cmpmode |= HSC3_MODE_STRMAP;

	if ( p1 & 16 ) {
		st = hsc3->CompileLabelOut( fname2, cmpmode );
	} else {
		if ( p3 )
			cmpmode |= HSC3_MODE_DEBUGWIN;
		if ( p1 & 8 ) {
...
EXPORT BOOL WINAPI hsc3_make( BMSCR *bm, char *p1, int p2, int p3 )
{
	...
	if ( type == 2 )
		strcat( ici_target, ".scr" );
	else
		strcat( ici_target, ".exe" );
#endif
