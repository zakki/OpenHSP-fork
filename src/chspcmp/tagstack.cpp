...
int CTagStack::StrCmp( const char *str1, const char *str2 )
{
...
int CTagStack::SearchTagID( char *tag )
{
	...
	for ( i = 0; i < tagent; i++ ) {
		if ( StrCmp( mem_tag[i].name, tag ) != 0 ) {
			return i;
		}
	}
	return -1;
...
CTagStack::CTagStack() : tagent( 0 ), lastidx( 0 ), gcount( 0 )
{
	for ( auto &i : mem_tag ) {
		i.name[0] = 0;
		i.uid = 0;
