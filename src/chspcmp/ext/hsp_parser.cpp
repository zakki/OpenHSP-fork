...
void CHspParser::CalcCG_token_exprbeg()
{
...
void CHspParser::CalcCG_token_exprbeg_redo()
{
...
OperatorType CHspParser::CalcCG_regmark( int mark )
{
	//		演算子を登録する
	//
	// writer->PutCSMark( mark, texflag );
	calccount++;
	return OperatorType( mark, texflag );
}

std::unique_ptr<ExpressionNode> CHspParser::CalcCG_factor()
{
	...
	case TK_NUM: {
		...
		auto node = std::make_unique<LiteralIntNode>( token.val, texflag );
		texflag = 0;
		CalcCG_token();
		calccount++;
		return node;
	...
	case TK_DNUM: {
		...
		auto node = std::make_unique<LiteralDoubleNode>( token.val_d, texflag );
		texflag = 0;
		CalcCG_token();
		calccount++;
		return node;
	...
	case TK_STRING: {
		...
		auto node = std::make_unique<LiteralStringNode>( token.cg_str, texflag );
		texflag = 0;
		CalcCG_token();
		calccount++;
		return node;
	...
	case TK_LABEL: {
		auto node = GenerateCodeLabel( token.cg_str, texflag );
		texflag = 0;
		CalcCG_token();
		calccount++;
		return node;
	...
	case TK_OBJ: {
		...
		auto node = GenerateCodeVAR( id, texflag );
		texflag = 0;
		if ( token.ttype == TK_NONE ) {
...
std::unique_ptr<ExpressionNode> CHspParser::CalcCG_unary()
{
	//		単項演算子
	//
	if ( token.ttype == '-' ) {
		...
		auto left = CalcCG_unary();
		texflag = 0;
		// writer->PutCS( TYPE_INUM, -1, texflag );
		auto right = std::make_unique<LiteralIntNode>( -1, texflag );
		return std::make_unique<BinaryOpNode>( CalcCG_regmark( '*' ), std::move( left ), std::move( right ) );
	} else {
		return CalcCG_factor();
	}
}

std::unique_ptr<ExpressionNode> CHspParser::CalcCG_muldiv()
{
	auto left = CalcCG_unary();
	...
	while ( ( token.ttype == '*' ) || ( token.ttype == '/' ) || ( token.ttype == '\\' ) ) {
		...
		auto right = CalcCG_unary();
		left = std::make_unique<BinaryOpNode>( CalcCG_regmark( op ), std::move( left ), std::move( right ) );
	}

	return left;
}

std::unique_ptr<ExpressionNode> CHspParser::CalcCG_addsub()
{
	auto left = CalcCG_muldiv();
...
std::unique_ptr<ExpressionNode> CHspParser::CalcCG_shift()
{
	auto left = CalcCG_addsub();
...
std::unique_ptr<ExpressionNode> CHspParser::CalcCG_compare()
{
	auto left = CalcCG_shift();
...
std::unique_ptr<ExpressionNode> CHspParser::CalcCG_bool()
{
	auto left = CalcCG_compare();
...
std::unique_ptr<ExpressionNode> CHspParser::CalcCG_start()
{
	//		entry point
	return CalcCG_bool();
}

std::unique_ptr<ExpressionNode> CHspParser::CalcCG( int ex )
{
	//		パラメーターの式を評価する
	//		(結果は逆ポーランドでコードを出力する)
	//
	texflag = ex;
	cs_lastptr = writer->cs_buf->GetSize();
	calccount = 0;
...
std::vector<std::unique_ptr<ExpressionNode>> CHspParser::GenerateCodePRM()
{
		...
		if ( token.ttype == TK_NONE ) {
			if ( token.val == ',' ) { // 先頭が','の場合は省略
				if ( ( ex & EXFLG_2 ) != 0 ) {
					...
					params.push_back( std::make_unique<LiteralMarkNode>( '?', EXFLG_2 ) );
				}
				token = lexer.GetTokenCG( GETTOKEN_DEFAULT );
				ex |= EXFLG_2;
				continue;
		...
		if ( ( compopt->hed_cmpmode & CMPMODE_OPTPRM ) != 0 ) {
			if ( calccount == 1 ) { // パラメーターが単一項目の時
				...
				case TK_STRING: {
					...
					// cstmp = (unsigned short *)( writer->cs_buf->GetBuffer() + cs_lastptr );
...
std::unique_ptr<ParameterList> CHspParser::GenerateCodePRMF()
{
	//		HSP3Codeを展開する(カッコ内のパラメーター)
	//		(戻り値 : exflg)
	//
	auto params = std::make_unique<ParameterList>( '(', std::vector<std::unique_ptr<ExpressionNode>>() );
	int ex = 0;
	while ( true ) {
		...
		if ( token.ttype == TK_NONE ) {
			if ( token.val == ')' ) { // ')'の場合は終了
				if ( ex != 0 ) {
					// writer->PutCS( TYPE_MARK, '?', EXFLG_2 );
					params->params.push_back( std::make_unique<LiteralMarkNode>( '?', EXFLG_2 ) );
				}
				return params;
			}
			if ( token.val == ',' ) { // 先頭が','の場合は省略
		...
		auto node = CalcCG( ex );
		params->params.push_back( std::move( node ) );
...
std::unique_ptr<ParameterList> CHspParser::GenerateCodePRMF2()
{
	...
	while ( true ) {
		...
		case TK_NUM: {
			...
			params->params.push_back( std::make_unique<LiteralIntNode>( token.val, ex ) );
			token = lexer.GetTokenCG( GETTOKEN_NOFLOAT );
			break;
		}
		case TK_OBJ: {
			...
			std::unique_ptr<ExpressionNode> node = std::make_unique<LiteralSymbolNode>( id, ex );
			token = lexer.GetTokenCG( GETTOKEN_DEFAULT );
			...
			if ( token.ttype == TK_NONE ) {
				if ( token.val == '(' ) { // '(' 配列指定
					...
					auto prms = GenerateCodePRMF();
					if ( prms->access_type != '(' ) {
						logger->Mesf( "#Bad ( access" );
						throw CGERROR_ARRAYEXP;
					}
					// writer->PutCS( TYPE_MARK, ')', 0 );
					token = lexer.GetTokenCG( GETTOKEN_DEFAULT );
					node = std::make_unique<ArrayAccessNode>( std::move( node ), std::move( prms ) );
				}
			}
...
std::unique_ptr<ParameterList> CHspParser::GenerateCodePRMF3()
{
	...
	auto node = GenerateCodeVAR( id, ex );
...
std::unique_ptr<VariableNode> CHspParser::GenerateCodePRMF4( int t, std::unique_ptr<LiteralSymbolNode> node )
{
	//		HSP3Codeを展開する(構造体/配列指定パラメーター)
	//
	auto var = std::make_unique<VariableNode>( std::move( node ) );

	if ( token.ttype == TK_NONE ) {
		if ( token.val == '.' ) {
			token = lexer.GetTokenCG( GETTOKEN_NOFLOAT );
			// writer->PutCS( TYPE_MARK, '(', 0 ); // '.' 配列指定
			auto params = GenerateCodePRMF2();
...
std::unique_ptr<ParameterList> CHspParser::GenerateCodeMethod()
{
	//		HSP3Codeを展開する(->に続くメソッド名)
	//
	auto params = std::make_unique<ParameterList>( '\0', std::vector<std::unique_ptr<ExpressionNode>>() );
	int id;
	int ex = 0;
	...
	case TK_STRING:
		...
		params->params.push_back( std::make_unique<LiteralStringNode>( token.cg_str, ex ) );
		token = lexer.GetTokenCG( GETTOKEN_DEFAULT );
		break;
	case TK_OBJ:
		id = SetVarsFixed( token.cg_str, cg_defvarfix );
		params->params.push_back( GenerateCodeVAR( id, ex ) );
		break;
	default:
		throw CGERROR_SYNTAX;
	}
...
std::unique_ptr<ExpressionNode> CHspParser::GenerateCodeLabel( const std::string &keyname, int ex )
{
	//		HSP3Codeを展開する(ラベル)
	//
	char lname[128];
	...
	int id = symtab->lb->Search( cname );
	if ( id < 0 ) { // 仮のラベル
		// int i = PutOT( -1 );
		int i = -1;
		id = symtab->lb->Regist( cname, TYPE_XLABEL, i );
	} else {
		int t = symtab->lb->GetType( id );
		if ( ( t != TYPE_XLABEL ) && ( t != TYPE_LABEL ) ) {
	...
	return std::make_unique<LiteralLabelNode>( id, ex );
}


std::unique_ptr<ExpressionNode> CHspParser::GenerateCodeVAR( int id, int ex )
{
	//		HSP3Codeを展開する(変数ほか)
	//		(idはlabel ID)
	//
	...
	//
	// writer->PutCSSymbol( id, ex );
	auto node = std::make_unique<LiteralSymbolNode>( id, ex );
	token = lexer.GetTokenCG( GETTOKEN_DEFAULT );
	...
	return GenerateCodePRMF4( t, std::move( node ) ); // 構造体/配列のチェック
}


void CHspParser::CheckCMDIF_Set( int mode )
{
...
void CHspParser::CheckCMDIF_Fin( int mode )
{
...
void CHspParser::CheckInternalIF( int opt )
{
	//		内蔵命令生成時チェック
	//
...
void CHspParser::CheckInternalListenerCMD( int opt )
{
	//		命令生成時チェック(命令+命令セット)
	//
	int i;
	...
	if ( o == 0x001 ) { // gosub
						// writer->PutCS( t, o & 0xffff, 0 );
	}
	token = lexer.GetTokenCG( GETTOKEN_DEFAULT );
}


int CHspParser::CheckInternalProgCMD( int opt, int orgcs )
{
	//		内蔵プログラム命令生成時チェック
	//
	int i;
	...
	case 0x0b: // (foreach)
		...
		// writer->PutCS( TK_LABEL, i, 0 );
		if ( opt == 0x0b ) {
			// writer->PutCS( TYPE_PROGCMD, 0x0c, EXFLG_1 );
			// writer->PutCS( TK_LABEL, i, 0 );
		}
		break;
	case 0x05: // repeat end
		...
		i = repend[replev];
		if ( i != -1 ) {
			// writer->SetOT( i, writer->GetCS() );
			repend[replev] = -1;
		}
		replev--;
		break;
	case 0x11: // stop
		// i = writer->PutOT( orgcs );
		// writer->PutCS( TYPE_PROGCMD, 0, EXFLG_1 );
		// writer->PutCS( TYPE_LABEL, i, 0 );
		break;
	case 0x19: // on
		token = lexer.GetTokenCG( GETTOKEN_DEFAULT );
		CalcCG( 0 ); // 式の評価
		...
		}
		{
			...
			if ( ( labelType == TYPE_PROGCMD ) && ( labelOpt == 0 || labelOpt == 1 ) ) {
				// goto or gosub
				// writer->PutCS( labelType, labelOpt, EXFLG_2 );
			} else {
				throw CGERROR_SYNTAX;
			}
		}
	...
	case 0x0f: // dupptr
	{
		...
		//	変数の初期化フラグをセットする
		symtab->lb->SetInitFlag( i, LAB_INIT_DONE );
...
void CHspParser::GenerateCodeCMD( int id )
{
	//		HSP3Codeを展開する(コマンド)
	//		(idはlabel ID)
	//
...
std::unique_ptr<StatementNode> CHspParser::GenerateCodeLET( int id, bool first )
{
	//		HSP3Codeを展開する(代入)
	//		(idはlabel ID)
	//
	...
	if ( ( token.ttype == TK_NONE ) && ( token.val == 0x65 ) ) { // ->が続いているか?
		writer->PutCS( TYPE_PROGCMD, 0x1a, EXFLG_1 );			 // 'mcall'コマンドに置き換える
		writer->PutCS( t, symtab->lb->GetOpt( id ), 0 );		 // 変数パラメーター
		token = lexer.GetTokenCG( GETTOKEN_DEFAULT );
		auto params = GenerateCodeMethod(); // パラメーター展開
		return nullptr;						// TODO std::make_unique<MethodCallNode>( std::move( params ) );
	}

	auto var = std::make_unique<LiteralSymbolNode>( id, EXFLG_1 );
	// writer->PutCS( t, symtab->lb->GetOpt( id ), EXFLG_1 ); // 通常の変数代入
	auto node = GenerateCodePRMF4( t, std::move( var ) ); // 構造体/配列のチェック
	...
	case '-': // '--'
		logger->Mesf( "[%c]", op );
		if ( token.ttype >= TK_SEPARATE ) {
			return std::make_unique<AssignmentNode>( optype, std::move( node ) );
		}
		if ( token.ttype == TK_NONE ) {
			if ( token.val == op ) {
				token = lexer.GetTokenCG( GETTOKEN_DEFAULT );
	...
	case '=': { // 変数=prm
		auto params = GenerateCodePRM();
		return std::make_unique<AssignmentNode>( optype, std::move( node ), std::move( params ) );
	}
	default:
		break;
	}
...
void CHspParser::GenerateCodePP_regcmd()
{
	...
	case TK_STRING:
		...
		writer->PutHPI( HPIDAT_FLAG_TYPEFUNC, 0, cmd2, cmd );
		cg_typecnt++;
		break;
	case TK_NUM:
		writer->PutHPI( HPIDAT_FLAG_SELFFUNC, 0, "", "" );
		cg_pptype = token.val;
		break;
	case TK_NONE:
		if ( token.val == '*' ) {
...
void CHspParser::GenerateCodePP_cmd()
{
	//		HSP3Codeを展開する(cmd)
	//
	int id;
	...
	id = symtab->lb->Regist( cmd, cg_pptype, id, lexer.cg_orgfilefull, lexer.cg_orgline );
	GenerateLabelListAndTag( id, LABBUF_FLAG_EXCMD );
	// logger->Mesf( "#%x:%d [%s]",cg_pptype, id, cmd );
}


void CHspParser::GenerateCodePP_uselib()
{
...
void CHspParser::GenerateCodePP_usecom()
{
	...
	*clsname = 0;
	...
	writer->SetLIBIID( cg_libindex, clsname );
	cg_libmode = CG_LIBMODE_COM;

	writer->PutStructStart();
	prmid = writer->PutStructEndDll( "*", cg_libindex, STRUCTPRM_SUBID_COMOBJ, -1 );
	int id =
		symtab->lb->Regist( libname, TYPE_DLLCTRL, prmid | TYPE_OFFSET_COMOBJ, lexer.cg_orgfilefull, lexer.cg_orgline );
	GenerateLabelListAndTag( id, LABBUF_FLAG_EXCMD );
...
void CHspParser::GenerateCodePP_func( int deftype )
{
	//		HSP3Codeを展開する(func)
	//
	int warn;
	...
	if ( cg_libmode == CG_LIBMODE_DLLNEW ) { // 初回はDLL名を登録する
		cg_libindex = writer->PutLIB( LIBDAT_FLAG_DLL, cg_libname );
	...
	if ( token.ttype == TK_NUM ) {
		...
		writer->PutStructParam( p1, STRUCTPRM_SUBID_STID );
		writer->PutStructParam( p2, STRUCTPRM_SUBID_STID );
		writer->PutStructParam( p3, STRUCTPRM_SUBID_STID );
		writer->PutStructParam( p4, STRUCTPRM_SUBID_STID );
		...
		while ( true ) {
			...
			writer->PutStructParam( t, STRUCTPRM_SUBID_STID );
			token = lexer.GetTokenCG( GETTOKEN_DEFAULT );
	...
	i = writer->PutStructEndDll( fname, cg_libindex, subid, otflag );
	int id = symtab->lb->Regist( fbase, TYPE_DLLFUNC, i, lexer.cg_orgfilefull, lexer.cg_orgline );
	GenerateLabelListAndTag( id, LABBUF_FLAG_EXCMD );

	// logger->Mesf( "#func [%s][%s][%d]",fbase, fname, i );
}

void CHspParser::GenerateCodePP_comfunc()
{
	...
	writer->PutStructStart();
	writer->PutStructParam( MPTYPE_IOBJECTVAR, STRUCTPRM_SUBID_STID );
	...
	i = writer->PutStructEndDll( "*", cg_libindex, subid, imp_index );
	int id = symtab->lb->Regist( fbase, TYPE_DLLCTRL, i | TYPE_OFFSET_COMOBJ, lexer.cg_orgfilefull, lexer.cg_orgline );
	GenerateLabelListAndTag( id, LABBUF_FLAG_EXCMD );
...
void CHspParser::GenerateCodePP_deffunc0( int is_command )
{
	...
	while ( true ) {
		...
		if ( ( t == MPTYPE_MODULEVAR ) || ( t == MPTYPE_IMODULEVAR ) || ( t == MPTYPE_TMODULEVAR ) ) {
			...
			prm = (STRUCTPRM *)writer->mi_buf->GetBuffer();
			...
			if ( t == MPTYPE_IMODULEVAR ) {
				...
				prm[symtab->lb->GetOpt( i )].offset = GET_FI_SIZE();
				regflag = 0;
			}
			...
			prmid = writer->PutStructParam( t, subid );
			token = lexer.GetTokenCG( GETTOKEN_DEFAULT );

		} else {
			prmid = writer->PutStructParam( t, STRUCTPRM_SUBID_STACK );
			...
			if ( token.ttype == TK_OBJ ) {
				...
				i = symtab->lb->Regist( token.cg_str, TYPE_STRUCT, prmid, lexer.cg_orgfilefull, lexer.cg_orgline );
	...
	// ot = writer->PutOT( writer->GetCS() );
	if ( index == -1 ) {
		index = GET_FI_SIZE();
		// writer->fi_buf->PreparePtr( sizeof( HED_STRUCTDAT ) );
		if ( regflag != 0 ) {
			symtab->lb->Regist( funcname, TYPE_MODCMD, index, lexer.cg_orgfilefull, lexer.cg_orgline );
		}
	}
	if ( label_id >= 0 ) {
	...
	// writer->PutStructEnd( index, funcname, dat_index, ot, funcflag );
}


void CHspParser::GenerateCodePP_deffunc()
{
	GenerateCodePP_deffunc0( 1 );
}


void CHspParser::GenerateCodePP_defcfunc()
{
	GenerateCodePP_deffunc0( 0 );
}


void CHspParser::GenerateCodePP_module()
{
...
void CHspParser::GenerateCodePP_struct()
{
	...
	// writer->PutStructStart();
	prmid = writer->PutStructParamTag(); // modinit用のTAG
	symtab->lb->Regist( funcname, TYPE_STRUCT, prmid, lexer.cg_orgfilefull, lexer.cg_orgline );
	...
	while ( true ) {
		...
		// prmid = writer->PutStructParam( t, STRUCTPRM_SUBID_STID );
		// logger->Mesf( "%d:type%d",prmid,t );
		...
		symtab->lb->Regist( token.cg_str, TYPE_STRUCT, prmid, lexer.cg_orgfilefull, lexer.cg_orgline );
	...
	// writer->PutStructEnd( funcname, STRUCTDAT_INDEX_STRUCT, 0, 0 );
}


void CHspParser::GenerateCodePP_defvars( int fixedvalue )
{
	...
	while ( true ) {
		...
		id = symtab->lb->Search( token.cg_str );
		if ( id >= 0 ) {
...
int CHspParser::SetVarsFixed( const std::string &varname, int fixedvalue )
{
	//		変数の固定型を設定する
	//
	int id;
	id = symtab->lb->Search( varname );
	if ( id < 0 ) {
		id = symtab->lb->Regist( varname, TYPE_VAR, cg_valcnt, lexer.cg_orgfilefull, lexer.cg_orgline );
		cg_valcnt++;
	}
...
void CHspParser::GenerateCodePP( char *buf )
{
	...
	if ( token.ttype == TK_NONE ) { // プリプロセッサから渡される行情報
		...
		if ( token.ttype == TK_STRING ) {
			...
			if ( compopt->cg_debug() ) {
				// writer->PutDI( 254, 0, lexer.cg_orgline ); // ラインだけをデバッグ情報として登録
			}
		}
...
int CHspParser::GenerateCodeSub()
{
	//		文字列(１行単位)からHSP3Codeを展開する
	//		(エラー発生時は例外が発生します)
	//
	std::vector<std::unique_ptr<StatementNode>> statements; // TOD return statements;
	...
	case TK_OBJ: {
		...
		if ( i < 0 ) {
			...
			auto node = GenerateCodeLET( i, true );
			logger->Mesf( "%s", serializer.Dump( *node ).c_str() );
...
int CHspParser::GenerateCodeBlock()
{
...
void CHspParser::RegisterFuncLabels()
{
...
void CHspParser::ResetGenerator( unsigned char *ptr )
{
	cg_flag = CG_FLAG_ENABLE;
	cg_valcnt = 0;
...
int CHspParser::GenerateCodeMain( CMemBuf *buf )
{
	//		ソースをHSP3Codeに展開する
	//		(ソースのバッファを書き換えるので注意)
	//
...
int CHspParser::GenerateCodeMainSkipError( CMemBuf *buf )
{
...
int CHspParser::GenerateCode( const std::string &fname, const std::string &oname, int mode )
{
	CMemBuf srcbuf;
...
int CHspParser::GenerateCode( CMemBuf *srcbuf, const std::string &oname, int mode )
{
	//		ファイルをHSP3Codeに展開する
	...
	if ( res != 0 ) {
		...
		CMemBuf srctmp;
#ifdef JPNMSG
		logger->Mesf( "%s(%d) : error %d : %s (%d行目)", lexer.cg_orgfile, lexer.cg_orgline, res,
					  cg_geterror( (CGERROR)res ), lexer.cg_orgline );
#else
		logger->Mesf( "%s(%d) : error %d : %s (line %d)", lexer.cg_orgfile, lexer.cg_orgline, res,
					  cg_geterror( (CGERROR)res ), lexer.cg_orgline );
#endif
		if ( cg_errline > 0 ) {
...
void CHspParser::CG_MesLabelDefinition( int label_id )
{
	if ( compopt->cg_debug() ) {
		return;
	}
...
char *CHspParser::GetLabelListHeader( int flag )
{
...
void CHspParser::GenerateLabelTag( const std::string &name, int flag, int type, const std::string &fname, int line )
{
...
void CHspParser::GenerateLabelListAndTag( int labelid, int flag )
{
	if ( labbuf == nullptr ) {
		return;
	}
	...
	LABOBJ *lab;
	lab = symtab->lb->GetLabel( labelid );
	if ( lab == nullptr ) {
		return;
	}
...
void CHspParser::GenerateLabelListAndTag( const std::string &name, int flag )
{
	if ( labbuf == nullptr ) {
		return;
	}
	GenerateLabelTag( name, flag, 0, lexer.cg_orgfilefull, lexer.cg_orgline );
}


void CHspParser::GenerateLabelListAndTagRef( int labelid, int flag )
{
	if ( labbuf == nullptr ) {
		return;
	}
	...
	GenerateLabelTag( lab->name, flag | LABBUF_FLAG_REFER, lab->type, lexer.cg_orgfilefull, lexer.cg_orgline );
}


void CHspParser::SetLabelListBuffer( CMemBuf *buf, int mode, char *match, int line, char *filename )
{
	labbuf = buf;
	cg_labout_mode = mode;
...
CHspParser::CHspParser( std::shared_ptr<CompileOptions> compopt, std::shared_ptr<CLogger> log )
	: CCompilerUtil( compopt ), logger( log ), lexer( compopt, log )
{
	symtab = std::make_shared<CSymbolTable>();
	serializer.SetSymbolTable( symtab );
	ResetCompiler();
}


CHspParser::~CHspParser() = default;
