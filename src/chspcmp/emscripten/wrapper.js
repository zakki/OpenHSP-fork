...
/**
 * int compile(const std::string& source, const std::string& filename, int mode, int ppopt, int dbgopt)
 ...
 */
hspcmp.compile = function(source, filename="source.hsp", mode=0, ppopt=0, dbgopt=0) {
    console.log("compile");
    let st = hspcmp.Module.compile(source, filename, mode, ppopt, dbgopt);
    let mes = hspcmp.Module.getMessage();
    let ax = null;
    if (st == 0) {
        let oname = hspcmp.Module.getObjectName();
        ax = hspcmp.Module.FS.readFile(oname);
    }
...
hspcmp.SAMPLE_SOURCE = `; block3.hsp
#include "hsp3dish.as"
