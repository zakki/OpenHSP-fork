...
void CToken::CalcCG_compare( void )
{
	...
	while ( ( ttype == '<' ) || ( ttype == '>' ) || ( ttype == '=' ) || ( ttype == '!' ) || ( ttype == 0x61 ) ||
			( ttype == 0x62 ) ) {
		op = ttype;
		CalcCG_token_exprbeg();
		if ( is_statement_end( ttype ) )
...
int CToken::CheckInternalProgCMD( int opt, int orgcs )
{
	...
	case 0x0f: // dupptr
	{
		...
		if ( firstSymbolName == NULL || isdigit( *reinterpret_cast<unsigned char *>( firstSymbolName ) ) )
			break;
...
int CToken::GenerateCode( CMemBuf *srcbuf, char *oname, int mode )
{
	...
	if ( res ) {
...
#else
		Mesf( "%s(%d) : error %d : %s (line %d)", cg_orgfile, cg_orgline, res, cg_geterror( (CGERROR)res ),
			  cg_orgline );
#endif
		if ( cg_errline > 0 ) {
		...
		if ( hed_option & HEDINFO_IORESUME )
		...
		if ( res < 0 ) {
			...
			Mesf( "#Vars (%d) Labels (%d) Modules (%d) Libs (%d) Plugins (%d)", cg_valcnt, ot_size >> 2, n_mod, li_size,
				  n_hpi );
			if ( sz_exopt ) {
...
void CToken::GenerateLabelTag( char *name, int flag, int type, char *fname, int line )
{
	...
	if ( cg_labout_match != NULL ) {
		if ( cg_labout_mode & LABLIST_MODE_PARTMATCH ) {
			if ( strstr2( name, cg_labout_match ) == NULL )
				return;
		} else {
