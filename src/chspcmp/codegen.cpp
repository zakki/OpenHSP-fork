...
void CCodeGenerator::CalcCG_token()
{
...
void CCodeGenerator::CalcCG_token_exprbeg()
{
	lexer.GetTokenCG( GETTOKEN_EXPRBEG );
	if ( token->ttype == TK_NONE ) {
		token->ttype = token->val;
	}
}

void CCodeGenerator::CalcCG_token_exprbeg_redo()
{
...
void CCodeGenerator::CalcCG_regmark( int mark )
{
	//		演算子を登録する
	//
...
void CCodeGenerator::CalcCG_unary()
{
	//		単項演算子
	//
	if ( token->ttype == '-' ) {
...
void CCodeGenerator::CalcCG_muldiv()
{
	int op;
	CalcCG_unary();
...
void CCodeGenerator::CalcCG_addsub()
{
	int op;
	CalcCG_muldiv();
...
void CCodeGenerator::CalcCG_shift()
{
	int op;
	CalcCG_addsub();
...
void CCodeGenerator::CalcCG_compare()
{
	int op;
	CalcCG_shift();
...
void CCodeGenerator::CalcCG_bool()
{
	int op;
	CalcCG_compare();
...
void CCodeGenerator::CalcCG_start()
{
	//		entry point
	CalcCG_bool();
}

void CCodeGenerator::CalcCG( int ex )
{
	//		パラメーターの式を評価する
	//		(結果は逆ポーランドでコードを出力する)
	//
...
void CCodeGenerator::GenerateCodePRM()
{
...
int CCodeGenerator::GenerateCodePRMF()
{
...
void CCodeGenerator::GenerateCodePRMF2()
{
...
int CCodeGenerator::GenerateCodePRMF4( int t )
{
...
void CCodeGenerator::GenerateCodeMethod()
{
...
void CCodeGenerator::GenerateCodeVAR( int id, int ex )
{
	//		HSP3Codeを展開する(変数ほか)
	//		(idはlabel ID)
	//
...
void CCodeGenerator::CheckCMDIF_Set( int mode )
{
...
void CCodeGenerator::CheckCMDIF_Fin( int mode )
{
...
void CCodeGenerator::CheckInternalIF( int opt )
{
	//		内蔵命令生成時チェック
	//
...
void CCodeGenerator::CheckInternalListenerCMD( int opt )
{
	//		命令生成時チェック(命令+命令セット)
	//
	int i;
...
int CCodeGenerator::CheckInternalProgCMD( int opt, int orgcs )
{
	//		内蔵プログラム命令生成時チェック
	//
	int i;
	...
	case 0x0f: // dupptr
	{
		char *firstSymbolName = lexer.GetSymbolCG( lexer.cg_ptr );
		if ( firstSymbolName == nullptr || ( isdigit( *reinterpret_cast<unsigned char *>( firstSymbolName ) ) != 0 ) ) {
			break;
		}
...
void CCodeGenerator::GenerateCodeCMD( int id )
{
	//		HSP3Codeを展開する(コマンド)
	//		(idはlabel ID)
	//
...
void CCodeGenerator::GenerateCodeLET( int id, bool first )
{
	//		HSP3Codeを展開する(代入)
	//		(idはlabel ID)
	//
...
void CCodeGenerator::GenerateCodePP_regcmd()
{
...
void CCodeGenerator::GenerateCodePP_cmd()
{
	//		HSP3Codeを展開する(cmd)
	//
	int id;
...
void CCodeGenerator::GenerateCodePP_uselib()
{
...
void CCodeGenerator::GenerateCodePP_usecom()
{
...
void CCodeGenerator::GenerateCodePP_func( int deftype )
{
	//		HSP3Codeを展開する(func)
	//
	int warn;
...
void CCodeGenerator::GenerateCodePP_comfunc()
{
...
void CCodeGenerator::GenerateCodePP_deffunc0( int is_command )
{
...
void CCodeGenerator::GenerateCodePP_deffunc()
{
	GenerateCodePP_deffunc0( 1 );
}


void CCodeGenerator::GenerateCodePP_defcfunc()
{
	GenerateCodePP_deffunc0( 0 );
}


void CCodeGenerator::GenerateCodePP_module()
{
...
void CCodeGenerator::GenerateCodePP_struct()
{
...
void CCodeGenerator::GenerateCodePP_defvars( int fixedvalue )
{
...
void CCodeGenerator::GenerateCodePP( char *buf )
{
	//		HSP3Codeを展開する(プリプロセスコマンド)
	//
	int i;
	lexer.GetTokenCG( GETTOKEN_DEFAULT ); // 最初の'#'を読み飛ばし
...
int CCodeGenerator::GenerateCodeSub()
{
	...
	lexer.GetTokenCG( GETTOKEN_LABEL );
	if ( token->ttype >= TK_SEPARATE ) {
		return token->ttype;
	}
...
int CCodeGenerator::GenerateCodeBlock()
{
	...
	if ( res == TK_SEPARATE ) {
		...
		if ( a1 == '{' ) { // when '{'
			...
			ff = 0;
			p = lexer.GetTokenCG( lexer.cg_ptr, GETTOKEN_DEFAULT );

			if ( token->ttype == TK_EOL ) { // 次行のコマンドがelseかどうか調べる
				if ( lexer.cg_wp != nullptr ) {
					p = lexer.GetSymbolCG( (char *)lexer.cg_wp );
					if ( p != nullptr ) {
						id = symtab->lb->Search( p );
						if ( id >= 0 ) {
...
void CCodeGenerator::RegisterFuncLabels()
{
...
int CCodeGenerator::GenerateCodeMain( CMemBuf *buf )
{
	//		ソースをHSP3Codeに展開する
	//		(ソースのバッファを書き換えるので注意)
	//
...
int CCodeGenerator::GenerateCodeMainSkipError( CMemBuf *buf )
{
...
void CCodeGenerator::CG_MesLabelDefinition( int label_id )
{
	if ( compopt->cg_debug() ) {
		return;
	}
...
char *CCodeGenerator::GetLabelListHeader( int flag )
{
...
void CCodeGenerator::ResetCompiler()
{
	//		reset labout
	cg_labout_mode = 0;
	cg_labout_line = 0;
...
void CCodeGenerator::GenerateLabelListAndTag( int labelid, int flag )
{
	if ( labbuf == nullptr ) {
		return;
	}
...
void CCodeGenerator::GenerateLabelListAndTagRef( int labelid, int flag )
{
	if ( labbuf == nullptr ) {
		return;
	}
...
void CCodeGenerator::SetLabelListBuffer( CMemBuf *buf, int mode, char *match, int line, char *filename )
{
	labbuf = buf;
	cg_labout_mode = mode;
...
CCodeGenerator::CCodeGenerator( std::shared_ptr<CompileOptions> compopt, std::shared_ptr<CLogger> log )
	: CCompilerUtil( compopt ), logger( log ), lexer( compopt, log ), token( &lexer.token )
{
