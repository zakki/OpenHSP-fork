...
{
	auto *errbuf = reinterpret_cast<CMemBuf *>( opaque );
	if ( errbuf != nullptr && msg != nullptr ) {
		errbuf->PutStr( msg );
		errbuf->PutCR();
	}
}

static void print_error( CMemBuf *errbuf, const char *msg )
{
	if ( errbuf != nullptr && msg != nullptr ) {
		errbuf->PutStr( msg );
		errbuf->PutCR();
...
static int compile_one_library_with_libtcc( CMemBuf *errbuf, const std::filesystem::path &native_path,
											...
											const ChspNativeArtifact &artifact )
{
	...
	if ( tcc == nullptr ) {
		print_error( errbuf, "#libtcc initialization failed." );
		return -1;
	}

	tcc_set_error_func( tcc, errbuf, chsp_tcc_error_handler );
	...
	if ( tcc_add_include_path( tcc, repo_root_str.c_str() ) < 0 ) {
		tcc_delete( tcc );
		print_error( errbuf, "#libtcc failed to add repository include path." );
		return -1;
	}
#ifdef HSPLINUX
	if ( tcc_add_library( tcc, "m" ) < 0 ) {
		tcc_delete( tcc );
		print_error( errbuf, "#libtcc failed to link libm." );
		return -1;
	}
#endif
	for ( const auto &lib : artifact.linked_libraries ) {
		if ( tcc_add_library( tcc, lib.c_str() ) < 0 ) {
			tcc_delete( tcc );
			print_error( errbuf, "#libtcc failed to link requested cHSP library." );
			return -1;
		}
	}
	...
	if ( tcc_add_file( tcc, native_path_str.c_str() ) < 0 ) {
		tcc_delete( tcc );
		print_error( errbuf, "#libtcc failed to compile generated cHSP C source." );
		return -1;
	}
	const std::string output_path_str = output_path.string();
	if ( tcc_output_file( tcc, output_path_str.c_str() ) < 0 ) {
		tcc_delete( tcc );
		print_error( errbuf, "#libtcc failed to write cHSP shared library." );
		return -1;
	}
	tcc_delete( tcc );
	return 0;
...
int chsp_compile_library_with_libtcc( CMemBuf *errbuf, const std::vector<std::string> &native_files, const char *compath,
									  const std::vector<ChspNativeArtifact> &native_artifacts )
{
	if ( native_files.empty() || native_files.size() != native_artifacts.size() ) {
		print_error( errbuf, "#Invalid cHSP native output list for libtcc compilation." );
		return -1;
	}

	std::filesystem::path repo_root;
	if ( !derive_repo_root_from_compath( compath, repo_root ) ) {
		print_error( errbuf, "#Failed to resolve repository root for libtcc include path." );
		return -1;
	}
	...
	for ( size_t i = 0; i < native_files.size(); ++i ) {
		...
		if ( compile_one_library_with_libtcc( errbuf, native_path, repo_root, output_path, native_artifacts[i] ) != 0 ) {
			return -1;
		}
	}
	return 0;
