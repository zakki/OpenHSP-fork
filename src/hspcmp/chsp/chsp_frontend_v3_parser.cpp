...
{
	const auto trimmed = chsputil::NormalizeIdentifier( chsputil::Trim( line ) );
	if ( chsputil::StartsWith( trimmed, "#chsp_module_end" ) ) {
		return chspv3::ChspV3SourceDirectiveKind::ModuleEnd;
	}
	if ( chsputil::StartsWith( trimmed, "#chsp_module" ) ) {
		return chspv3::ChspV3SourceDirectiveKind::Module;
	}
	if ( chsputil::StartsWith( trimmed, "#chsp_cdecl" ) ) {
		return chspv3::ChspV3SourceDirectiveKind::ChspCDecl;
	}
	if ( chsputil::StartsWith( trimmed, "#chsp_clink" ) ) {
		return chspv3::ChspV3SourceDirectiveKind::ChspCLink;
	}
	if ( chsputil::StartsWith( trimmed, "#chsp_c" ) ) {
		return chspv3::ChspV3SourceDirectiveKind::ChspC;
	}
	if ( chsputil::StartsWith( trimmed, "#chsp_defcfunc" ) ) {
		return chspv3::ChspV3SourceDirectiveKind::DefCFunc;
	}
	if ( chsputil::StartsWith( trimmed, "#chsp_deffunc" ) ) {
	...
	while ( true ) {
		...
		if ( ( t == MPTYPE_MODULEVAR ) || ( t == MPTYPE_IMODULEVAR ) || ( t == MPTYPE_TMODULEVAR ) ) {
			...
			if ( token.ttype == TK_OBJ ) {
				...
				i = symtab->lb->Regist( const_cast<char *>( token.cg_str.c_str() ), TYPE_STRUCT, prmid, lexer.cg_orgfilefull.c_str(),
										lexer.cg_orgline );
...
void CChspParser::RegisterLocalStructAlias( const std::string &name )
{
	...
	id = symtab->lb->Regist( const_cast<char *>( name.c_str() ), TYPE_STRUCT, 0, lexer.cg_orgfilefull.c_str(), lexer.cg_orgline );
...
void CChspParser::GenerateCodePP_chsp_cdecl()
{
	...
	current_module->declared_native_functions.push_back( token.cg_str );
...
void CChspParser::GenerateCodePP_struct()
{
	...
	while ( true ) {
		...
		symtab->lb->Regist( const_cast<char *>( token.cg_str.c_str() ), TYPE_STRUCT, prmid, lexer.cg_orgfilefull.c_str(), lexer.cg_orgline );
...
int CChspParser::SetVarsFixed( const std::string &varname, int fixedvalue )
{
	...
	if ( id < 0 ) {
		id = symtab->lb->Regist( const_cast<char *>( varname.c_str() ), TYPE_VAR, cg_valcnt, lexer.cg_orgfilefull.c_str(), lexer.cg_orgline );
		cg_valcnt++;
	}
...
int CChspParser::GenerateCodeSub()
{
	...
	if ( current_module == nullptr ) {
		// Non-directive lines outside cHSP modules are passed through to upstream hspcmp.
		return TK_EOL;
	}
...
void CChspParser::PreScanFunctions( const char *ptr )
{
	...
	while ( std::getline( stream, line ) ) {
		line_no++;
		std::string trimmed = chsputil::Trim( line );
		if ( chsputil::StartsWith( trimmed, "#chsp_deffunc" ) ) {
			std::istringstream ls( trimmed.substr( 13 ) );
			std::string name;
			if ( ls >> name ) {
				...
				if ( symtab->lb->Search( name_ptr ) < 0 ) {
					symtab->lb->Regist( name_ptr, TYPE_MODCMD, -1, const_cast<char *>( lexer.cg_orgfilefull.c_str() ), line_no );
				}
			}
...
int CChspParser::SearchSymbol( const std::string &name )
{
	int id = symtab->lb->Search( const_cast<char *>( name.c_str() ) );
	if ( id < 0 && name.find( '@' ) == std::string::npos ) {
		std::string hsp_name = name + "@hsp";
		id = symtab->lb->Search( const_cast<char *>( hsp_name.c_str() ) );
	}
	return id;
}
...
int CChspParser::GenerateCodeMain( CMemBuf *buf )
{
	...
	PreScanFunctions( const_cast<const char *>( buf->GetBuffer() ) );
	int a;
