...
std::pair<const char *, CCgToken> CCgLexer::GetTokenCG( const char *str, int option )
{
	...
	if ( ( ( option & GETTOKEN_EXPRBEG ) != 0 ) && a1 == '-' ) {
		if ( isdigit( to_uchar( vs[1] ) ) ) {
			char next2 = static_cast<char>( tolower( to_uchar( vs[2] ) ) );
			if ( !( vs[1] == '0' && ( next2 == 'x' || next2 == 'b' ) ) ) {
...
const char *CCgLexer::GetLineCG()
{
	...
	while ( true ) {
		...
		if ( a1 == 13 || a1 == 10 ) {
			size_t last_pos = current_line_buffer.find_last_not_of( " \t" );
			if ( last_pos != std::string::npos && current_line_buffer[last_pos] == '\\' ) {
				current_line_buffer.erase( last_pos );
				current_line_buffer += ' ';
				p++;
				line++;
				cg_orgline++;
